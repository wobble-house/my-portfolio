    ...
    return(
        <nav className="flex fixed z-50 md:justify-center top-0 w-full">
            <div className="flex flex-col-reverse md:flex-row flex-grow justify-end">
                <button className="z-50 fixed md:hidden px-2 border-2 rounded-md border-primary focus:border-2 focus:animate-pulse mr-auto mt-4 ml-5 transition-all ease-in-out duration-300" onMouseDown={handleMainNavButton}>
                  {navbar ? (
                ...
                <div ref={Navref} className={`flex mx-auto md:mx-0 bg-foreground md:bg-transparent md:block  ${navbar ? 'fixed top-0 flex-col min-h-content items-center w-full h-full transition-all ease-in-out pt-12' : 'hidden'}`}>
                    <ul className="text-center flex flex-col md:flex-row text-accent md:border-b-4 md:border-x-4 border-primary md:rounded-b-md">
                        <Logo mobile={true} position="pb-6 justify-center"/>
